/* _denchou_kanji_hover_v0.1.0.js */
(function () {
    ...
    if (preloadedKanjiData[kanji]) {
      ...
      for (const wordInfo of relatedWords) {
        ...
        if (wordInfo.sentence) {
          const autoQuote = window.denchouConfig && window.denchouConfig.kanjiHoverAutoQuote === 'true';
        ...
        if (wordInfo.pitchPosition) {
          ...
          if (numbers.length > 0) {
            ...
            const relevantPitchTypes = numbers.map(num => window.denchouGetPitchType(num, tempRt).split(',')[0].trim());
