/* _denchou_kanji_hover_v0.4.0.js */
(function () {
  if (
    window.denchouConfig &&
    window.denchouConfig.enableKanjiHover === "false"
  ) {
    return;
  }
  ...
  function drawTooltipPitchLines(tooltip) {
    ...
    entries.forEach((li) => {
      ...
      for (let i = 0; i < moraCount; i++) {
        ...
        if (type === "high") {
          if (
            (i + 1 < moraCount && pattern[i + 1] === "L") ||
            (pClass === "odaka" && i === moraCount - 1)
          ) {
  ...
  async function ankiConnectRequest(action, params = {}) {
    try {
      ...
      console.error("[DenChou Hover] AnkiConnect Failure:", error);
      return null;
    }
  }
  ...
  function positionTooltipNearElement(element, targetElement) {
    ...
    if (
      element.classList.contains("tooltip-above") &&
      finalTop + parentRect.top < 0
    ) {
    ...
    newArrowLeft = Math.max(
      arrowSize,
      Math.min(newArrowLeft, tooltipWidth - arrowSize),
    );
  ...
  async function showKanjiInfo(element) {
    ...
    if (preloadedKanjiData[kanji]) {
      ...
      const kanjiHighlightRegex = new RegExp(
        kanji.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&"),
        "g",
      );
      ...
      for (const wordInfo of relatedWords) {
        const wordHtml = wordInfo.word.replace(
          kanjiHighlightRegex,
          highlightSpan,
        );
        ...
        if (wordInfo.sentence) {
          const autoQuote =
            window.denchouConfig &&
            window.denchouConfig.kanjiHoverAutoQuote === "true";
        ...
        if (wordInfo.pitchPosition) {
          ...
          if (wordInfo.reading) {
            ...
            const readingTextContent =
              tempReadingDiv.textContent || tempReadingDiv.innerText || "";
          ...
          if (numbers.length > 0) {
            formattedPositionHtml = numbers
              .map((num, index) => {
                let pitchType = window.denchouGetPitchType(num, tempRt);
                pitchType = pitchType.split(",")[0].trim();
                return `<span class="pitch-number ${pitchType}">${num}</span>`;
              })
              .join("・");

            const relevantPitchTypes = numbers.map((num) =>
              window.denchouGetPitchType(num, tempRt).split(",")[0].trim(),
            );
            firstPitchCategory = relevantPitchTypes[0] || "";
            fullCategoriesHtml = relevantPitchTypes.join("<br>");
          } else {
            firstPitchCategory = wordInfo.pitch
              ? wordInfo.pitch.split(",")[0].trim()
              : "";
            fullCategoriesHtml = wordInfo.pitch
              ? wordInfo.pitch.replace(/,/g, "<br>")
              : "";
          }
        } else {
          firstPitchCategory = wordInfo.pitch
        ...
        const clickAction = `onclick="window.denchouAnkiRequest('guiBrowse', { query: 'nid:${wordInfo.noteId}' })"`;
        const itemHtml = `
      ...
      tooltipElement.innerHTML =
        count === 0 ? "No words found." : relatedWordsHtml;
      ...
      if (showMoreBtn) {
        showMoreBtn.addEventListener("click", () => {
          const hiddenItems = tooltipElement.querySelectorAll(
      ...
      tooltipElement.addEventListener("mouseover", (event) => {
        ...
        if (positionElement) {
          ...
          if (
            categoriesElement &&
            categoriesElement.classList.contains("categories") &&
  ...
  window.denchouAnkiRequest = ankiConnectRequest;

  // init
  window.denchouInitKanjiHover = async function () {
    ...
    if (kanjiSpans.length > 0) {
      wordText = Array.from(kanjiSpans)
    ...
    await Promise.all(
      Array.from(uniqueKanji).map(async (kanji) => {
        if (preloadedKanjiData[kanji]) return;

        const query = `note:DenChou "word:*${kanji}*"`;
        const noteIds = await ankiConnectRequest("findNotes", { query });

        if (!noteIds || noteIds.length === 0) return;

        const notesInfo = await ankiConnectRequest("notesInfo", {
          notes: noteIds,
        });
        if (!notesInfo) return;
        ...
        const allCardIds = [];
        notesInfo.forEach((n) => {
          if (n.cards && n.cards.length) allCardIds.push(...n.cards);
        });

        const cardTypeMap = {};
        if (allCardIds.length > 0) {
          const cardsInfo = await ankiConnectRequest("cardsInfo", {
            cards: allCardIds,
          });
          if (cardsInfo) {
        ...
        for (const note of notesInfo) {
          ...
          if (fields && fields.word && fields.word.value) {
            ...
            if (
              wordValue.trim() !== wordText &&
              !uniqueWords.has(wordValue.trim())
            ) {
              const readingValueFull = fields.pitchAccents
                ? fields.pitchAccents.value
                : fields.reading
                  ? fields.reading.value
                  : "";
              const sentenceValue = fields.sentence
                ? fields.sentence.value
                : "";
              let readingHtml = "";
              ...
              const pitchValue = fields.pitchCategories
                ? fields.pitchCategories.value
                : fields.pitch
                  ? fields.pitch.value
                  : "";
              const pitchPositionValue = fields.pitchPositions
                ? fields.pitchPositions.value
                : fields.pitchPosition
      ...
      if (isKanji(textContent[i])) {
        if (lastIndex < i) {
          fragment.appendChild(
            document.createTextNode(textContent.slice(lastIndex, i)),
          );
        }
        const span = document.createElement("span");
    ...
    const extraNodes = Array.from(rubyElement.childNodes).filter(
      (node) =>
        node.nodeName.toLowerCase() === "rt" ||
        node.nodeName.toLowerCase() === "rp",
    );
